...
from pyspark.sql.functions import (
    col, count, sum, avg, stddev, hour, dayofweek, when, window, from_json
)
...
class FraudStreamingDetector:
    ...
    def create_streaming_features(self, df):
        ...
        df_featured = df_with_velocity.withColumn(
            ...
            "high_velocity_flag",
            when(col("trans_count_1h") > 10, 1).otherwise(0)
        )
    ...
    def process_stream(self):
        ...
        fraud_alerts = predictions \
            .withColumn("fraud_probability", 
                        col("probability").getItem(1)) \
            .withColumn("is_fraud", 
                        when(col("fraud_probability") > 0.7, 1).otherwise(0))
        ...
        def write_to_monitoring(df, epoch_id):
            ...
            if not df.isEmpty():
                ...
                alerts_pd.to_csv(
                    f"../data/alerts/fraud_alerts_{epoch_id}.csv", 
                    index=False
                )
        
        alert_query = high_risk.writeStream \
            .foreachBatch(write_to_monitoring) \
            .outputMode("append") \
            .trigger(processingTime="5 seconds") \
            .start()
        
        # 3. Aggregate statistics for dashboard
        stats_query = fraud_alerts \
    ...
    def send_fraud_alert(self, alert):
        ...
        with open("../data/alerts/fraud_log.txt", "a") as f:
            f.write(f"{datetime.now()}: {alert['transaction_id']} - Risk: {alert['fraud_probability']:.2%}\n")
